...
- Preserve meaningful destination paths and query parameters, including long URLs. Never truncate a destination to make a tool call succeed.
...
Return the short URL first. Then mention whether A.af reused an existing link and summarize any relevant campaign or analytics result concisely. Respond in the user's language; keep the plugin name and technical identifiers unchanged.
